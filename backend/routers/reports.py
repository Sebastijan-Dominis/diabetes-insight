...
def generate_pdf_report(explanation: str, result: str, title: str) -> FileResponse:
...
async def diabetes_diagnosis(data: DiabetesInput, request: Request):
    ...
    try:
        pdf = generate_pdf_report(explanation=explanation, result=result, title="Diabetes Diagnosis Report")
    except Exception as e:
...
async def diabetes_risk_score(data: RiskInput, request: Request):
    ...
    try:
        pdf = generate_pdf_report(explanation=explanation, result=result, title="Diabetes Risk Score Report")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"An error occurred during PDF generation: {str(e)}")
